...
for i = 1:num_cluster
    count_cluster(i) = numel(find(Y == centers(i)));
end
...
purity = 0;
for i = 1:num_cluster
    member = find(Y == centers(i));
    member_ids = PY(member);
...
for i = 1:N
    index_cluster = find(Y(i) == centers);
    index_item = item_map(PY(i));
